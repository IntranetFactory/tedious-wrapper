/* 
    based on https://github.com/tediousjs/node-mssql/blob/master/lib/connectionstring.js
    Copyright (c) 2013-2017 Patrik Simek, The MIT License
*/
'use strict'
...
const resolveConnectionString = function (string, driver) {
  ...
  const config = {
    ...
    options: {
      ...
      connectTimeout: oror(parsed.connectionTimeout, parsed.timeout, parsed['connect timeout'], parsed['connection timeout']),
  ...
  if (server) {
    ...
    if (/^(.*),(.*)$/.exec(server)) {
      ...
      config.options.port = parseInt(RegExp.$2.trim(), 10)
    }
